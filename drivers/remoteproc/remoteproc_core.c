...
static int rproc_resume(struct device *dev)
{
	...
	/*
	 * a message came after rproc was system suspended, so we need to
	 * resume and runtime status needs to be updated too, thas is easy
	...
	/*
	 * a system suspend request came when rproc was running, so the
	 * rproc was suspended but the runtime status is active. In this
...
mark:
	/* if a resume was needed we need to schedule the autosupend too */
