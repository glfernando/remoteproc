...
static int _resource_release(struct rprm *rprm, struct rprm_elem *e)
{
	...
	/* if there are constraints remove them */
				...
				int *res_id, void *data, size_t len)
{
	...
	dev_dbg(dev, "requesting %s resoruce len %d\n", name, len);
...
static int _printf_constraints_args(struct rprm_elem *e, char *buf, size_t len)
{
			 ...
			 size_t count, loff_t *ppos)
{
	...
	list_for_each_entry(e, &rprm->res_list, next) {
		...
		if (e->constraints.mask)
			c += _printf_constraints_args(e, buf + c, len - c);
...
static int rprm_probe(struct rpmsg_channel *rpdev)
{
	 ...
	 * which remote proc is using this resmgr connenction
	 */
