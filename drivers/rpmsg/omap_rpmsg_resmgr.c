...
static int rprm_gptimer_sprint(void *handle, char *buf)
{
	struct rprm_gpt_depot *gptd = handle;
...
static int rprm_sdma_sprint(void *handle, char *buf)
{
	struct rprm_sdma *sd = handle;
	int i, ret = 0;
...
static int rprm_auxclk_request(void **handle, void *args, size_t len)
{
	...
	if (!src) {
		pr_err("%s: unable to get aux source clock\n", __func__);
		ret = -EIO;
		goto error_aux;
	}
	...
	if (ret) {
		pr_err("%s: unable to set clk %s as parent of %s\n",
					__func__, auxck->pname, auxck->name);
		goto error_aux_parent;
	}

	ret = clk_set_rate(parent, auxck->pclk_rate);
	if (ret) {
		pr_err("%s: rate %u not supported by %s\n", __func__,
						auxck->pclk_rate, auxck->pname);
		ret = -EINVAL;
		goto error_aux_parent;
	}
...
static int rprm_auxclk_sprint(void *handle, char *buf)
{
	struct rprm_auxclk_depot *acd = handle;
...
MODULE_DESCRIPTION("Remote Processor Resource Manager OMAP resourses");
