...
static int _rproc_latency(void *handle, struct device *tdev, unsigned long val)
{
	struct rprm_rproc_depot *rprocd = handle;

	return rproc_set_constraints(tdev, rprocd->rp,
				     RPROC_CONSTRAINT_LATENCY, val);
}

static int _rproc_bandwidth(void *handle, struct device *tdev, unsigned long val)
{
	struct rprm_rproc_depot *rprocd = handle;
...
static int _rproc_scale(void *handle, struct device *tdev, unsigned long val)
{
	struct rprm_rproc_depot *rprocd = handle;
